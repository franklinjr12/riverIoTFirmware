...
time_t setClock()
{
  ...
  Serial.println("[NTP TIME] Waiting for NTP time sync: ");
  ...
  Serial.println("[NTP TIME] Current time: ");
...
static void initializeTime()
{
  ...
  // configTime(0, 3600, NTP_SERVERS);
  time_t now = time(NULL);
  const unsigned long timeout = 1000 * 60 * 2;
  unsigned long t1 = millis();
  unsigned long t2 = millis();
  ...
  if (t2 - t1 >= timeout)
    ESP.restart();
  Serial.println("");
  Serial.printf("now %ld time %ld diff %ld\n", now, UNIX_TIME_NOV_13_2017, now - UNIX_TIME_NOV_13_2017);
  // setClock();
